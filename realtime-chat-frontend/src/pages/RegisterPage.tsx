...
export const RegisterPage = () => {
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        if (username.length < 3 || username.length > 30 || !/^[a-zA-Z0-9_]+$/.test(username)) {
            toast.error("Username must be 3-30 chars and contain only letters, numbers, and underscores");
            return;
        }
            ...
            <div className="w-full max-w-md bg-bg-secondary/80 backdrop-blur-xl border border-border rounded-2xl shadow-2xl shadow-accent/10 p-8">
                ...
                <form onSubmit={onSubmit} className="space-y-4">
                    ...
                    <button
                        type="submit"
                        disabled={isLoading}
                        className="w-full bg-gradient-to-r from-accent to-accent-hover text-white font-semibold rounded-lg px-4 py-3 flex justify-center items-center hover:scale-[1.02] transition-transform disabled:opacity-70 disabled:hover:scale-100 shadow-lg shadow-accent/20 mt-2"
                    >
                        {isLoading ? (
